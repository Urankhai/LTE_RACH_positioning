...
for slowMode = 0:1
    ...
    opts.printEvery     = 25;
    [xk] = Mathwork_OMP( A, b, K_target, errFcn,opts);
    toc
...
tic
[xk] = Mathwork_OMP( {Af,At}, b, K_target, errFcn, opts );
toc
fprintf('Error is %.2e\t(oracle error is %.2e)\n\n', norm(xk-x)/norm(x), er_oracle );
...
disp(   '---------------------------------------');
fprintf('CoSaMP, -------------------------------\n\n');
[xk] = Mathwork_CoSaMP( A, b, K_target, errFcn, opts);
fprintf('Error is %.2e\t(oracle error is %.2e)\n\n', norm(xk-x)/norm(x), er_oracle );
...
[xk] = Mathwork_CoSaMP( {Af,At}, b, K_target, errFcn, opts);
fprintf('Error is %.2e\t(oracle error is %.2e)\n\n', norm(xk-x)/norm(x), er_oracle );