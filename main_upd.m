...
concrete = 0.3; % for 2.6GHz and the angle of incidence 45 degree
ref_coef = [1, concrete, 0.5*concrete];
...
ant_vect = 24;%[8, 16, 24, 32, 48, 64];
std_diff = zeros(5,length(ant_vect));
std_angle_diff = zeros(5, length(ant_vect));
PTx = 100; %mW
SNR14_dBm = 10*log10(PTx*(lambda/(4*pi*14000))^2); % dBm
SNR_global = [];

colors_main = {'k-o', 'k--o', 'k-.o', 'k:o', 'k-'};
lwidth = [3,3,3,3,1];

for scenario_ind = 1:5
...
%             close all
            ...
            Scenario_plain_upd        % reflection planes, coordinates of UE and BS are defined in scenario
            r_orgn(:,iii) = radius;
            ...
            for ant_i = 1:Ant_Num
                ...
                for k = 1:UE_Num
                    ...
                    rec_rach_noise = ref_coef(k)*LoS_UL_r + noise;
%                     figure(1)
%                     hold on
%                     plot(abs(ref_coef(k)*LoS_UL_r))
%                     keyboard
...
%         plot(angle_stat,'-o')
        ...
        std_diff(scenario_ind, ant_step) = std(r_stat);%(sum(r_diff)/N_iter);
        std_angle_diff(scenario_ind, ant_step) = std(angle_stat);%(sum(angle_diff)/N_iter);
...
std_diff1000 = [0.1385, 0.03677, 0.008251, 0.004306, 0.001422, 0.0014;
    0.789, 0.1859, 0.05302, 0.02586, 0.008724, 0.00432;
    3.21, 0.4304, 0.1807, 0.1079, 0.03778, 0.01388;
    9.778, 3.665, 0.7221, 0.2628, 0.1548, 0.1153;
    129.3, 20.97, 7.149, 2.676, 1.129, 0.5376];

figure(5000)
hold on
title('Average error of coordinates estimation')
for sc = 1:5
    plot(ant_vect, min(std_diff(sc,:),std_diff1000(sc,:)),colors_main{sc},'linewidth',lwidth(sc))
end
...
for sc = 1:5
    plot(ant_vect, std_angle_diff(sc,:), colors_main{sc}, 'linewidth',lwidth(sc))
end
legend('30-100','100-200','200-300','300-500','500-1000')
